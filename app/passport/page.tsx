  ...
  return (
    <Stack direction="row">
      <Stack direction="column" flex={1}>
        ...
        >
          <Box
            sx={{ borderBottom: '1px solid #383838', paddingY: "30px" }}
          >
            ...
            <div className="mt-10 grid grid-cols-1 gap-3 md:gap-5">
              <div onClick={() => router.push(`/passport/scanqr/${1}`)} className="col-span-1 cursor-pointer p-[10px] flex items-center rounded-[10px] bg-[rgba(255,255,255,0.05)]">
                <div className="mr-[10px]">
                  <QRCodeIcon />
                </div>
                <div className="">
                  <div className="mb-[4px] text-white text-[18px] leading-[120%] font-[700]">Scan QR Code</div>
                  <div className="text-[10px] leading-[120%] text-[rgba(255,255,255,0.7)]">Scan tickets</div>
                </div>
              </div>
          ...
          <Box sx={{ paddingY: "30px", borderBottom: '1px solid #383838' }}>
            {
              MOCK_DATA.eventCredential.map((item, index) => (
                <div onClick={() => router.push(`passport/${index}`)} key={index} className="bg-[rgba(255,255,255,0.02)] mb-2.5 last-of-type:mb-0 rounded-[10px] flex justify-between p-[10px]">
                  <div className="flex items-center">
                    <div className="mr-[14px]">
                      <Image alt={item.name} src={item.image} width={50} height={50} />
                    </div>
                    <div className="">
                      <div className="text-[18px] font-bold text-white leading-[120%] mb-[4px]">{item.name}</div>
                      <div className="text-[10px] leading-[120%] text-[rgba(255,255,255,0.7)]">{item.desc}</div>
                    </div>
