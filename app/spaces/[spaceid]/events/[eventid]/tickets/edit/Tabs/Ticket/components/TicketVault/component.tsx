...
export const WithdrawToken = ({tokenSymbol, balance, tokenAddress, ticketAddress}: IWithdrawToken) => {
  ...
  const handleWithdraw = async() => {
    ...
    try {
      const withdrawHash = await writeContract(config, {
        ...
        abi: TICKET_ABI,
        args: [withdrawInfo?.recipient, tokenAddress, parseUnits(String(withdrawInfo?.amount), decimal)]
      });

      const { status: withdrawStatus } = await waitForTransactionReceipt(config, {
        hash: withdrawHash,
      });
...
export const ConfirmWithdrawalTransaction = ({
  ...
  return (
    <>
      {showWithdrawalModal && (
        <Box>
          <Box display={'flex'} alignItems={'center'} marginTop={"30px"} marginBottom={"14px"}>
            <SendIcon />
